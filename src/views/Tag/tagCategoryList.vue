<template>
  <div class="tagcategory">
    ...
    <Row>
      <Table :loading="loading" :columns="tccolumns" :data="tcdata.data" stripe @on-sort-change="handleSorting"></Table>
    </Row>
    <Row >
      <div style="float: right;padding-top: 10px;">
        <Page v-if="tcdata.total > 10" :total="tcdata.total" :current="cpage" show-sizer show-total :page-size="limit" @on-change="pageChange" @on-page-size-change="psizeChange"></Page>
        <div v-else-if="tcdata.total === 1">Total {{tcdata.total}} item</div>
        <div v-else>Total {{tcdata.total}} items</div>
      </div>
   </Row>
  </div>
</template>
...
<script>
// import tagcategoryModel from '@/api/tagcategory'
...
export default {
                  ...
                  click: async() => {
                    ...
                    if (res.status === 'success') {
                    	if (res.msg === 'found') {
                  		  this.$Modal.confirm({
                          title: 'Confirm',
                          content: '<p><b style="color: #f90;">'+ res.data.length + ((res.data.length === 1) ? ' Tag' : ' Tags') +' </b> found with <b>'+ params.row.tc_name +'</b>. </p><p>Are you sure you want to '+(params.row.status ? 'INACTIVE': 'ACTIVE')+' all?</p>',
                          loading: true,
                          onOk: async () => {
                          	for (let item of res.data) {
                          		let s = await axios.patch(tagsUrl + '/' + item.id, {tag_status: !params.row.status})
                          	}
                          	axios.patch(tagcategoryUrl + '/' + params.row.id, {status: !params.row.status}).then(res => {
											        this.tcdata.data[params.index].status = !params.row.status
          ...
          render: (h, params) => {
            ...
            return h('div', [
	            	h('a', {
	            		...
	            		on: {
	            			click: () => {
	            				this.$emit('updateTag', {type: 'tagcategory', id: params.row.id})
	            			}
	            		}
	            	},[
	            		h('Icon', {
            	...
            	// }, [
	            	h('a', {
	            		...
	            		on: {
		                	...
		                	axios.get(tagsUrl + '?userId=' + userId + '&tag_category=' + params.row.id).then(res => {
		                		if (res.data.data.length > 0) {
		                			this.$Modal.confirm({
			                      title: 'Confirm',
			                      content: '<p><b style="color:#f90;">'+res.data.data.length + ' ' + ((res.data.data.length === 1) ? 'Tag': 'Tags') +'</b> found with Tag Category.</p><p>Are you sure you want to Delete all?</p>',
			                      loading: true,
			                      onOk: async() => {
			                        for(let item of res.data.data) {
			                        	let a = await axios.delete(tagsUrl + '/' + item.id)
			                        }
  ...
  methods: {
  	handleSorting (item) {
  		let inx = _.findIndex(this.tccolumns, {key: item.key})
  		this.tccolumns[inx].sortType = item.order
  		if (item.order !== 'normal') {
  			for (let [minx, obj] of this.tccolumns.entries()) {
    ...
    async handleTagClick(id, status, inx) {
    	let userId = Cookies.get('userDetailId')
    	let resp = await axios.get(tagsUrl + '?userId=' + userId + '&tag_category=' + id).then(res => {
    		if (res.data.data.length > 0) {
    ...
    handleDelete (iid) {
      let finx = _.findIndex(this.tcdata.data, {id: iid})
      if (finx !== undefined && finx >= 0) {
        axios.delete(tagcategoryUrl + '/' + iid).then(res => {
          this.tcdata.data.splice(finx, 1)
          this.tcdata.total -= 1
          ...
          // if (this.filterobj.name !== '') {
	      	// 	query += '&tc_name=' + this.filterobj.name 
	      	// }
	      	if (this.filterobj.website !== '') {
	      		query += '&website=' + this.filterobj.website 
	      	}
          axios.get(tagcategoryUrl + query).then(res => {
          	if (res.data.data.length > 0) {
          		this.tcdata.data.push(res.data.data[0])
          	} else {
          		this.tcdata.data.total -= 1
          	}
          })
        }).catch(err => {
    ...
    async init(item) {
      ...
      if (userId !== '' && userId !== undefined) {
	      ...
	      this.tcdata = await axios.get(tagcategoryUrl + query).then(res => {
	        return res.data
	      }).catch(err => {

...
export default {
    ...
    return {
      ...
      tcolumns: [
        ...
        },
        {
          ...
          sortType: 'normal',
          render: (h, params) => {
            let inx = _.findIndex(this.categoryOptions, {value: params.row.tag_category})
            let tName = ''
            if (inx !== -1) {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            const color = row.tag_status ? 'green' : 'red';
            const text = row.tag_status ? 'ACTIVE' : 'INACTIVE';
            ...
            return h('Tag', {
                ...
                nativeOn: { 
                  click: () => { 
                    this.handleTagClick(params.row.id, params.row.tag_status, params.index) 
                  } 
                }
            }, text)
          }
          ...
          render: (h, params) => {
            ...
            return h('div', [
                h('a', {
                  ...
                  on: {
                    click: () => {
                      this.$emit('updateTag', {type: 'tag', id: params.row.id})
                    }
                  }
                },[
                  h('Tooltip', {
                ...
                h('a', {
                  ...
                  on: {
                    click: () => {
                      this.$emit('updateTag', {type: 'productmapping', id: params.row.id, website: params.row.websiteId})
                    }
                  }
                },[
                  h('Tooltip', {
                ...
                h('a', {
                  ...
                  on: {
                    click: () => {
                      this.$emit('updateTag', {type: 'producttags', id: params.row.id, website: params.row.websiteId})
                    }
                  }
                },[
                  h('Tooltip', {
  ...
  methods: {
    ...
    handleTagClick(id, status, inx) {
      if(!status) {
        axios.get(tagcategoryUrl + '/' + this.tdata.data[inx].tag_category).then(res => {
          if (res.data.status) {
            axios.patch(tagsUrl + '/' + id, {tag_status: !status}).then(res => {
              this.tdata.data[inx].tag_status = !status
              this.$Notice.success({ title: 'Success!', desc: '', duration: 3})
              this.init()
            })
          } else {
            this.$Notice.warning({ title: 'Warning!!', desc: 'You want to first active tagcategory <b><u>' + res.data.tc_name + '</u></b>.', duration: 5})
          }
        }).catch(err => {
    ...
    handleDelete (iid) {
      ...
      if (finx !== undefined && finx >= 0) {
        axios.delete(tagsUrl + '/' + iid).then(res => {
          ...
          let userId = Cookies.get('userDetailId')
          axios.get(tagsUrl + '?userId=' + userId + '&$sort[createdAt]=-1&$skip=' + skp + '&$limit=1').then(res => {
    ...
    async init (item) {
      ...
      if (userId !== '' && userId !== undefined) {
        ...
        if (this.filterobj.category !== '') {
          query += '&tag_category=' + this.filterobj.category
        }
        if (this.filterobj.status !== '') {
          query += '&tag_status=' + JSON.parse(this.filterobj.status)
        }
        
        this.tdata = await axios.get(tagsUrl + query).then(res => {
  ...
  async mounted () {
    ...
    axios.get(tagcategoryUrl + '?userId=' + userId + '&$paginate=false').then(res=> {
      for(let item of res.data) {
        let websiteName = _.find(this.webOptions, {value: item.website})
        this.categoryOptions.push({
          label: item.tc_name,
          value: item.id,
          weblabel: item.tc_name + " [ website : " + websiteName.label + " ]"
        })
      }
    })

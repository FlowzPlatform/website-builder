<template>
  <div class="flyercategorynew">
   <Row>
      <div style="padding-bottom: 10px;">
        <h2 v-if="formItem.id">Edit Flyer Category</h2>
        <h2 v-else>Add Flyer Category</h2>
      </div>
   </Row>
   <Row style="border: 1px solid #C0C0C0; padding: 20px">
     <Form :model="formItem" :label-width="300" ref="formItem" :rules="rulesformItem">
        ...
        <FormItem label="Category Name" prop="fc_name">
            <Input v-model.trim="formItem.fc_name" placeholder="Flyer Category Name"></Input>
        </FormItem>
...
let flyerCategoryUrl = baseUrl + '/flyer-category'

export default {
  name: 'flyercategorynew',
  props: {
    fdata: Object
  },
  data () {
    const validatefcname = async(rule, value, callback) => {
      ...
      if (value !== '' && this.formItem.website !== '') {
        let resp = await (axios.get(flyerCategoryUrl + '?userId=' + userId +'&website=' + this.formItem.website + '&fc_name=' + value).then(res => {
          if (this.formItem.id) {
        ...
        if (resp.length > 0) {
          callback(new Error('Flyer Category Name already Exist in Selected Website. Please try another.'))
        } else {
          callback();
        }
      }
    ...
    return {
      ...
      rulesformItem: {
        ...
        fc_name: [
          { required: true, message: 'Please fill in the Flyer Category Name', trigger: 'blur' },
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          axios.post(flyerCategoryUrl, this.formItem).then(res => {
            this.$Notice.success({title: 'Success', desc: 'Successfully saved.', duration: 3})
            this.$emit('updateDocument', {type: 'flyercategorylist'})
          }).catch(err => {
    ...
    handleEdit (name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          axios.put(flyerCategoryUrl + '/' + this.formItem.id, item).then(res => {
            this.$Notice.success({title: 'Success!!', desc: 'Successfully Edited.', duration: 3})
            this.$emit('updateDocument', {type: 'flyercategorylist'})
            // this.$router.push('/flyercategory')
          }).catch(err => {
            console.log('Error', err)
          })
        }
  ...
  mounted () {
    ...
    if (!_.isEmpty(this.fdata)) {
      if (this.fdata.type === 'flyercategory' && this.fdata.id !== '') {
        axios.get(flyerCategoryUrl + '/' + this.fdata.id).then(res => {

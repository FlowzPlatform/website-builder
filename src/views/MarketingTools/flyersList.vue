...
export default {
      ...
      fcolumns: [
        {
          ...
          align: 'center',
          render: (h, params) => {
            let resp = params.row.flyer_name
            if (resp === '') {
              resp = '-'
            }
            return h('div', resp)
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('a', {
                attrs: {
                  href: params.row.flyer_image,
                  target: '_blank'
                }
              }, [
                h('img', {
                  attrs: {
                    src: params.row.flyer_image,
                    width: 130,
                    height: 65
                  },
                  class: {
        ...
        {
          title: 'Flyer Category',
          width: 220,
          key: 'flyer_category',
          sortable: true,
          sortType: 'normal',
          render: (h, params) => {
            let inx = _.findIndex(this.categoryOptions, {value: params.row.flyer_category})
            let tName = ''
            if (inx !== -1) {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            const color = row.flyer_status ? 'green' : 'red';
            const text = row.flyer_status ? 'ACTIVE' : 'INACTIVE';
            ...
            return h('Tag', {
                ...
                nativeOn: { 
                  click: () => { 
                    this.handleFlyerClick(params.row.id, params.row.flyer_status, params.index) 
                  } 
                }
            }, text)
          }
          ...
          render: (h, params) => {
            ...
            return h('div', [
                h('a', {
                  ...
                  on: {
                    click: () => {
                      this.$emit('updateDocument', {type: 'flyer', id: params.row.id})
                    }
                  }
                },[
                  h('Tooltip', {
  ...
  methods: {
    ...
    handleFlyerClick(id, status, inx) {
      if(!status) {
        axios.get(flyerCategoryUrl + '/' + this.tdata.data[inx].flyer_category).then(res => {
          if (res.data.status) {
            axios.patch(flyersUrl + '/' + id, {flyer_status: !status}).then(res => {
              this.tdata.data[inx].flyer_status = !status
              this.$Notice.success({ title: 'Success!', desc: '', duration: 3})
              this.init()
            })
          } else {
            this.$Notice.warning({ title: 'Warning!!', desc: 'You want to first active flyercategory <b><u>' + res.data.fc_name + '</u></b>.', duration: 5})
          }
        }).catch(err => {
    ...
    handleDelete (iid) {
      ...
      if (finx !== undefined && finx >= 0) {
        axios.delete(flyersUrl + '/' + iid).then(res => {
          ...
          let userId = Cookies.get('userDetailId')
          axios.get(flyersUrl + '?userId=' + userId + '&$sort[createdAt]=-1&$skip=' + skp + '&$limit=1').then(res => {
    ...
    async init (item) {
      ...
      if (userId !== '' && userId !== undefined) {
        ...
        if (this.filterobj.category !== '') {
          query += '&flyer_category=' + this.filterobj.category
        }
        if (this.filterobj.status !== '') {
          query += '&flyer_status=' + JSON.parse(this.filterobj.status)
        }
        
        this.tdata = await axios.get(flyersUrl + query).then(res => {
        ...
        for (let [inx, mitem] of this.tdata.data.entries()) {
          axios.get(flyerCategoryUrl + '/' + mitem.flyer_category).then(res => {
  ...
  async mounted () {
    ...
    axios.get(flyerCategoryUrl + '?userId=' + userId + '&$paginate=false').then(res=> {

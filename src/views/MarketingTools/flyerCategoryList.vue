<template>
  <div class="flyercategory">
    <Row>
      <div  style="padding-bottom: 10px;">
      	<h2>Flyer Categories List</h2>
      </div>
    </Row>
...
<script>
// import flyercategoryModel from '@/api/flyercategory'
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            return h('Tag', {
                ...
                nativeOn: { 
                  click: async() => {
                    let res = await this.handleFlyerClick(params.row.id, params.row.status, params.index)
                    if (res.status === 'success') {
                    	if (res.msg === 'found') {
                  		  this.$Modal.confirm({
                          title: 'Confirm',
                          content: '<p><b style="color: #f90;">'+ res.data.length + ((res.data.length === 1) ? ' Flyer' : ' Flyers') +' </b> found with <b>'+ params.row.fc_name +'</b>. </p><p>Are you sure you want to '+(params.row.status ? 'INACTIVE': 'ACTIVE')+' all?</p>',
                          loading: true,
                          onOk: async () => {
                          	for (let item of res.data) {
                          		let s = await axios.patch(flyersUrl + '/' + item.id, {flyer_status: !params.row.status})
                          	}
                          	axios.patch(flyerCategoryUrl + '/' + params.row.id, {status: !params.row.status}).then(res => {
          ...
          render: (h, params) => {
            ...
            return h('div', [
	            	h('a', {
	            		...
	            		on: {
	            			click: () => {
	            				this.$emit('updateDocument', {type: 'flyercategory', id: params.row.id})
	            			}
	            		}
	            	},[
	            		h('Icon', {
            	...
            	// }, [
	            	h('a', {
	            		...
	            		on: {
		                	...
		                	axios.get(flyersUrl + '?userId=' + userId + '&flyer_category=' + params.row.id).then(res => {
		                		if (res.data.data.length > 0) {
		                			this.$Modal.confirm({
			                      title: 'Confirm',
			                      content: '<p><b style="color:#f90;">'+res.data.data.length + ' ' + ((res.data.data.length === 1) ? 'Flyer': 'Flyers') +'</b> found with Flyer Category.</p><p>Are you sure you want to Delete all?</p>',
			                      loading: true,
			                      onOk: async() => {
			                        for(let item of res.data.data) {
			                        	let a = await axios.delete(flyersUrl + '/' + item.id)
			                        }
  ...
  methods: {
    ...
    async handleFlyerClick(id, status, inx) {
    	let userId = Cookies.get('userDetailId')
    	let resp = await axios.get(flyersUrl + '?userId=' + userId + '&flyer_category=' + id).then(res => {
    		if (res.data.data.length > 0) {
    ...
    handleDelete (iid) {
      ...
      if (finx !== undefined && finx >= 0) {
        axios.delete(flyerCategoryUrl + '/' + iid).then(res => {
          ...
          axios.get(flyerCategoryUrl + query).then(res => {
          	if (res.data.data.length > 0) {
    ...
    async init(item) {
      ...
      if (userId !== '' && userId !== undefined) {
	      ...
	      this.fcdata = await axios.get(flyerCategoryUrl + query).then(res => {
	        return res.data
	      }).catch(err => {

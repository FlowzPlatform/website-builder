<template>
  <div class="Login">
    ...
    <div class="row">
      ...
      <div class="col-md-2">
        <el-button type="primary" @click="inviteNow()">Send User Invitation</el-button>
      </div>
      </div>
    </div></div>
    </div>
    ...
    export default {
        ...
        methods: {
            async getDataOfSubscriptionUser(){
                let sub_id = []
                await axios.get('http://auth.flowzcluster.tk/api/userdetails',{ headers: { 'Authorization': Cookies.get('auth_token') } })
                .then(response => {
                    console.log("res.................------>>>>", response.data.data.package)
                    let obj_val = Object.values(response.data.data.package)
                    let first_key = obj_val[1].subscriptionId
                    let obj_key = Object.keys(response.data.data.package)
                    for (let index = 0; index < obj_val.length; index++) {
                      if (obj_val[index].role == "admin") {
                        sub_id.push({"value2":obj_val[index].subscriptionId, "label2":obj_val[index].subscriptionId})
                      }
                    }
            ...
            async inviteNow() {
                ...
                let params = {
                    ...
                    "role": {
                        "webbuilder": this.value
                    }
                }
              console.log("params........", params)
              console.log("config.inviteAPI........", config.inviteAPI)
                await axios({
                        method: 'POST',
                        url: config.inviteAPI,
                        headers: {

      ...
      if(this.configData.status == 200 || this.configData.status == 204){
        ...
        for(var i = 0; i < Object.keys(this.settings[1].pageSettings).length; i++){
          ...
          for(let j = 0; j < partialsList.length; j++){
            partialsListString += Object.keys(partialsList[j]) + ' => ' + Object.values(partialsList[j]) + ', ';
          }
...
.title-style-2{
  background: linear-gradient(to right, #04A5C1 0%, #F9F981 100%);
}

...
import domenu from 'domenu'
	...
	export default {
		...
		async mounted () {
			...
			try {
			    let responseConfig = await axios.get(config.baseURL + '/flows-dir-listing/0?path=' + folderUrl + '/assets/' + actualFileNameOnly + '.json',{ headers: { 'Authorization': this.$session.get('token') } } );
